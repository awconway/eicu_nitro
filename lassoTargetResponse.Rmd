---
title: "Lasso model response targets"
...
```{targets specLassoResponse, tar_simple = TRUE}
  linear_reg(
    penalty = tune(),
    mixture = 1
  ) |>
...
```{targets gridLassoResponse, tar_simple = TRUE }
...
```{targets recLassoResponse, tar_simple = TRUE }
  recipe(sbp_post ~
  sbp_pre +
    nitro_diff_mcg +
    total_nitro +
...
```{targets workflowLassoResponse, tar_simple = TRUE}
workflow()|>
    add_recipe(recLassoResponse)|>
    add_model(specLassoResponse)
```

```{targets tuningLassoResponse, tar_simple = TRUE}
    tune_grid(
